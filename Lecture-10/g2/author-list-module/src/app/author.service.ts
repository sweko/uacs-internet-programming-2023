...
export class AuthorService {
  ...
  getAuthors(): Observable<Author[]> {
    const url = 'https://raw.githubusercontent.com/sweko/uacs-internet-programming-exams/main/dry-run-mid-term/data/authors.json';
    const response = this.http
        .get<ApiAuthor[]>(url)
        .pipe(
          map((apiAuthors: ApiAuthor[]) => apiAuthors.map(this.toAuthor)),
          catchError((error: any) => of([] as Author[]))
        );
